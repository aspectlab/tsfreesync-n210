...
% cChanA=ChanA(1:2:end);
% cChanB=ChanB(1:2:end);


cChanA=ChanA(1:2:end)+1j*ChanA(2:2:end);
...
Rldif0 = zeros(length(cChanA)/PULSE_PER,1);
...
parfor n = 1:length(cChanA)/PULSE_PER
    ...
    % store offsets
    report = fine_offset1;
    Rldif0(n) =fine_offset1;
end


%% Calculate Statistics (in units of actual time, not in fractional samples)
R1MeanOff = mean(Rldif0)*(1/fsrx);
R1SD =      std(Rldif0)*(1/fsrx);
...
figure(421)
plot(Rldif0./ratio,'*')
title('quadratic estimator')
xlabel('time (in SPB''s)')
ylabel('measured offset (in fractional Tx samples)')
grid on

figure(2)  % check to make sure we're not half-way out of phase
